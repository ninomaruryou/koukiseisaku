...
// UNIT.17
Static_mesh_instncing::Static_mesh_instncing(ID3D11Device* device, const char* fbx_filename, bool triangulate,int MAX_INSTNCING)
{
	//UNIT30
	...
	//	deserialization(scene_view, meshes, materials); //シリアライズは不完全でも読み込むため注意が必要	
	...
	this->MAX_INSTNCING = MAX_INSTNCING;
	// UNIT.18
	create_com_objects(device, fbx_filename);
}
// UNIT.18
void Static_mesh_instncing::fetch_meshes(FbxScene* fbx_scene, std::vector<mesh>& meshes)
{
...
// UNIT.18
void Static_mesh_instncing::create_com_objects(ID3D11Device* device, const char* fbx_filename)
{
	// UNIT.18
	...
	Texture::create_vs_from_cso(device, "Static_mesh_instncing_vs.cso", vertex_shader.ReleaseAndGetAddressOf(), input_layout.ReleaseAndGetAddressOf(), input_element_desc, ARRAYSIZE(input_element_desc));
	...
	_ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
		...
		buffer_desc = {};
	buffer_desc.ByteWidth = sizeof(instance) * MAX_INSTNCING * meshes.size();
	buffer_desc.Usage = D3D11_USAGE_DYNAMIC;
	buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	buffer_desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	buffer_desc.MiscFlags = 0;
	buffer_desc.StructureByteStride = 0;
	instance* instances = new instance[MAX_INSTNCING * meshes.size()];
...
void Static_mesh_instncing::Begin(ID3D11DeviceContext* immediate_context)
{
	HRESULT hr = S_OK;
...
// UNIT.25
void Static_mesh_instncing::render(ID3D11DeviceContext* immediate_context, const XMFLOAT4X4& world, const XMFLOAT4& material_color)
{
	for (mesh& mesh : meshes)
	{
		...
		instances[count_instance].color = material_color;
		count_instance++;
...
void Static_mesh_instncing::End(ID3D11DeviceContext* immediate_context)
{
	...
	for (mesh& mesh : meshes)
	{
		...
		for (const mesh::subset& subset : mesh.subsets)
		{
			...
			XMStoreFloat4(&data.material_color, XMLoadFloat4(&DirectX::XMFLOAT4(1, 1, 1, 1)) * XMLoadFloat4(&material.Kd));
...
// UNIT.19
void Static_mesh_instncing::fetch_materials(FbxScene* fbx_scene, std::unordered_map<uint64_t, material>& materials)
{
...
bool Static_mesh_instncing::raycast(const DirectX::XMFLOAT4& position, const DirectX::XMFLOAT4& direction,
