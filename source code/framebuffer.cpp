...
void framebuffer::clear(ID3D11DeviceContext* immediate_context, float r, float g, float b, float a, float depth)
{
	float color[4]{ r, g, b, a };
	 immediate_context->ClearRenderTargetView(render_target_view.Get(), color);//画面の塗りつぶし
	 immediate_context->ClearDepthStencilView(depth_stencil_view.Get(), D3D11_CLEAR_DEPTH, depth, 0);//	クリアする深度ステンシルへのポインター（表示範囲の設定？） （この値は 0 から 1 の間でクランプ。）
